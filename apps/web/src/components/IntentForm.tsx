...
export function IntentForm({
  defaults,
  onSubmit,
}: { defaults: { network: string }; onSubmit: (v: IntentFormValue) => void }) {
  const [v, setV] = useState<IntentFormValue>({
    agent: '',
    receiver: '',
    token: '',
    contract: '',
  ...
  function submit(e: React.FormEvent) {
    e.preventDefault();
    if (!ACCOUNT_HASH.test(v.agent))
      return setErr('agent must be an account-hash hex (00<64 hex>)');
