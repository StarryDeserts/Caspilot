...
`<CaspilotMark variant="lockup|glyph|mono|onlight">` using the **refined** geometry: ring `M39.7 27.3 A16 16 0 1 0 37.6 15.5` (~44° C-gap, `currentColor`, `vector-effect="non-scaling-stroke"`), needle polygon `42.3,20.6 28.2,22.0 24,24 26.6,26.6 20.8,25.7` (~12° lance, `var(--accent)`), pivot `r=2.1` currentColor, tip-tick `rotate(-12 …)`. `role="img"` + `<title>Caspilot</title>`. **favicon.svg** = simplified 32-viewBox (ring `M26.8 18.3 A11 11 0 1 0 25.3 10.2` stroke 4, shuttle polygon `27.3,14.0 18.6,14.6 16,16 17.6,17.7 14.6,17.0`, no pivot dot). Wordmark = Bricolage 600, `-0.01em`. Placements per `Logo 集成方案`: sidebar (22–24px), landing footer (16–18px), developers topbar (mono 20px), doc footer (16px); add `<link rel="icon" type="image/svg+xml">`. Hero stays glyph-free. Do **not** add the mark to wallet/health/state badges/table rows.
