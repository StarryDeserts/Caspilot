...
- **Phase 6 — Tier-1 demo harness (real on-chain proof): not started.** Blocked on a write-capable Casper adapter — the Phase-4 adapters are read-only, so a `put_deploy` / WASM-submission path plus local PEM signing must be built before the harness can broadcast.
